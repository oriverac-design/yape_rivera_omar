...
| **CÓDIGO DEL ESTUDIANTE:** | 2221895826
| **NÚMERO DE CLASE:** |4 |
|---|---|---|---|
| **APELLIDOS Y NOMBRES:** |RIVERA CASTILLO, Omar | **FECHA ENTREGA:** | 27/06/2026|
...
|------------------------|-------------------|--------------------|--------------------------------------------|
| Core de pagos (3.2M transacciones/día, no puede perder dinero) | PostgreSQL / CockroachDB| Relacional / NewSQL| Garantiza consistencia estricta (ACID) inmediata y aislamiento total de transacciones financieras, evitando pérdidas de dinero o saldos fantasmas|
| Sesiones de login activo (15M usuarios, expira en 30 min) |Redis |NoSQL / Clave-Valor (In-Memory | Ofrece lecturas y escrituras en submilisegundos al almacenar datos en RAM, soportando el alto rendimiento|
| Perfil del comerciante (bodega, restaurante, taxi — atributos distintos) | MongoDB|NoSQL / Documental (JSON) |cada tipo de comercio guarda atributos específicos (platos, placas, categorías) en un mismo contenedor sin necesidad de alterar la estructura global. |
| Historial de transacciones para análisis (18 TB/año) |Apache Spark + Delta Lake (Databricks) | Data Lakehouse / Almacenamiento analítico| Permite procesar de forma masiva y distribuida los 18 TB anuales con rendimiento óptimo|
| Red de detección de fraude (ciclo A→B→C→A en < 5 min) |Neo4j| NoSQL / Orientada a Grafos| Facilita el análisis de relaciones complejas, caminos y detección de ciclos cerrados de transferencias sospechosas|
| Dashboard ejecutivo (top 10 distritos, actualización diaria) |ClickHouse o PostgreSQL |BD Analítica | Optimiza la agregación y cálculo de reportes analíticos diarios sobre millones de filas a gran velocidad.|
...
|------------|----------------|----------------------|----------------------------------------------------------------|
| Core de pagos (débito/crédito de saldos) | CP (Consistencia / Tolerancia a la Partición)| Disponibilidad| Es correcto. En transacciones financieras la consistencia debe ser absoluta; si hay una partición de red, es preferible rechazar temporalmente el pago|
| Historial "mis últimas 50 transacciones" |AP (Disponibilidad / Tolerancia a la Partición) |Consistencia Estricta |Es correcto. Si el sistema sufre una caída de red, el usuario valora más poder abrir su app y ver su historial rápido (alta disponibilidad) |
...
Resuelve el problema del escalado vertical obligatorio y los costos exagerados de hardware. Con Oracle, para soportar tanta gente estás amarrado a un único servidor principal gigante que cuesta una fortuna mejorar. En cambio, CockroachDB permite aplicar un escalado horizontal real; es decir, simplemente vas sumando computadoras normales y baratas a la red para aguantar los 50 millones de usuarios sin que el sistema se vuelva lento ni pierda datos.
b) ¿Por qué MongoDB NO puede reemplazar a Oracle para el procesamiento de pagos aunque también escala horizontalmente?

Porque MongoDB nació originalmente pensando en la rapidez y en que los datos se actualicen "con el paso de los segundos" (consistencia eventual). Aunque hoy en día maneja transacciones, no tiene la rigidez extrema, el aislamiento ni los bloqueos automáticos que necesita el motor de pagos de un banco. Si dos personas intentaran mover el mismo saldo al mismo milisegundo, MongoDB podría generar un descuadre de dinero (condición de carrera), algo que Oracle o una base de datos relacional jamás permitirían.
...
Utiliza el Protocolo de Consenso Raft (apoyado por el sistema de control de concurrencia MVCC).
---
...
Lo usaría cuando los desarrolladores de Yape necesiten escribir código, hacer pruebas rápidas o experimentos en sus propias computadoras sin depender de una conexión a internet. Docker permite levantar una base de datos local limpia en segundos, probar que todo funcione bien y destruirla sin generar gastos de servidores en la nube ni alterar los datos de producción de la empresa

b) ¿Qué ventaja tiene Atlas M0 sobre el contenedor Docker para el contexto universitario?

La gran ventaja de Atlas M0 es que corre directo en la nube y no consume nada de memoria RAM ni procesador de tu computadora, lo cual es ideal si tienes una laptop viejita o con pocos recursos. Además, te da una interfaz web súper amigable para ver los datos sin tener que instalar programas pesados, y te permite compartir tu base de datos con el profesor mediante un simple enlace de internet.
...
Si ejecutas docker rm, el contenedor local se borra por completo de la computadora y todos los datos que guardaste ahí se pierden para siempre. En cambio, con MongoDB Atlas los datos están totalmente a salvo en los servidores web; no importa si apagas tu computadora, borras programas o cierras la sesión, la información en la nube se queda guardada y guardando su persistencia de manera permanente.
